...
using Catalog.Application.Common.Resolvers;
...
public class AggregateRepository<TA, TKey> :
    IAggregateRepository.IAggregateRepository<TA, TKey> where TA : class, IAggregateRoot<TKey>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _eventStoreClient.AppendToStreamAsync(
            streamName, StreamState.Any, eventList.ToArray(),
            cancellationToken: cancellationToken);
    }

    public async Task<Dictionary<int, object>> ReadEventsAsync(
        TKey aggregateId, CancellationToken cancellationToken = default)
    {
        var streamName = GetStreamName(aggregateId);
        var result = _eventStoreClient
            .ReadStreamAsync(
                Direction.Forwards,
                streamName,
        ...
        foreach (var data in await result.ToListAsync(cancellationToken: cancellationToken))
        {
            ...
            var eventMetaData =
                JsonSerializer.Deserialize<EventMeta>(
            ...
            var eventInfo = JsonConvert.DeserializeObject(jsonData, typeInfo,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var aggregateResult = BaseAggregateRoot<TA, TKey>
                .Create(events.OrderBy(x => x.AggregateVersion));
